...
function Restaurant(props) {
  return (
    <TouchableHighlight onPress={()=>_onRestaurantPress(props.gps)} underlayColor="white">
      <View style={styles.restaurant}>
        <Text style={{fontSize: 20}}>{props.name}</Text>
        <Text style={{fontSize: 18}}>{props.type}</Text>
        <Text style={{fontSize: 12}}>วันทำการ {props.open_day}    เวลาทำการ {props.open_time}</Text>
  ...
  _onPressButton() {
    if(this.state.restaurantData) {
      ...
      for (let i = 0; i < restaurantData.length; i++){
        ...
        res_type = restaurantData[i].type
        var name_result = res_name.match(patt);
        var type_result = res_type.match(patt)
        if (name_result || type_result) {
